...
class PositionManager:
    ...
    def calculate_quantity(self, price: float, position_pct: float, stop_loss: float = 0) -> float:
        ...
        fixed_qty = round(self.account_value * (capped_pct / 100.0) / price, 4) if price > 0 else 0
        ...
        if stop_loss > 0 and stop_loss < price:
            ...
            risk_qty = round(max_risk_dollars / risk_per_share, 4) if risk_per_share > 0 else fixed_qty
